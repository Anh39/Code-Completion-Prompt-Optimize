...
def prepare_test_repo_environment(repo_root_path, conda_dir, env_dir):
    ...
    for repo_name in repo_names:
        ...
        env_path = f"{env_dir}/envs/repo_{repo_name}"
        repo_path = f"{repo_root_path}/{repo_name}"
        check = False
        if not os.path.exists(env_path):
            subprocess.run(f"{conda} create -p {env_path} python=3.9 -y", shell=True, check=check)
        # os.environ["PATH"] = f"{env_dir}/envs/repo_{repo_name}/bin:" + os.environ["PATH"]
        if os.path.exists(f"{repo_path}/requirements.txt"):
            # subprocess.run(f"cd {repo_path}; pip install -r requirements.txt", shell=True)
            subprocess.run(f"{conda} run -p {env_path} bash -c 'cd {repo_path} && pip3 install -r requirements.txt'", shell=True, check=check)
        ...
        subprocess.run(f"{conda} run -p {env_path} bash -c 'cd {repo_path} && pip3 install -e .'", shell=True, check=check)
        # subprocess.run(f"pip install pytest pytest-cov pytest-benchmark coincidence cssutils docutils openpyxl hypothesis pycodestyle", shell=True)  
