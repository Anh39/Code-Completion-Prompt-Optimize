...
from utils.data_loader import load_data_repoeval
...
def _generate(args):
    ...
    for task in tasks:
        ...
        dataset = load_data_repoeval(task_prompt_file)
        prompts = []
        print(f"Running on {task} task")
        print(f"Preparing data :")
        for sample in tqdm(dataset):
            prefix = sample["prompt"]
            suffix = sample.get("full_right_context", sample.get("right_context"))
        ...
        file_name = f"repoeval-{task}.jsonl"
...
def generate(custom_args: Optional[argparse.Namespace] = None):
    ...
    if custom_args is None:
        ...
        parser.add_argument("--model_output_dir", type=str, default="model_outputs/repoeval")
        parser.add_argument("--eval_output_dir", type=str, default="eval_outputs/repoeval")
    ...
    for task in obj.tasks:
        ...
        src_path = os.path.join(args.model_output_dir, f"repoeval-{task}.jsonl")
        dst_path = os.path.join(folder_path, "prediction.jsonl")
        os.makedirs(folder_path, exist_ok=True)
        shutil.copy(src_path, dst_path)
    compute_metric_stmt(obj)
    shutil.copy(f"{obj.output_dir}/{obj.dataset}/results.json", os.path.join(args.eval_output_dir, f"repoeval.json"))
