...
class RepoLevelDataset(Dataset):
    ...
    def _apply_fim_original(self, text: str):
        ...
        mid_index = len([self.fim_prefix] + prefix + [self.fim_suffix] + suffix)
        return total, mid_index
    def __getitem__(self, idx):
        row = self._data_adapter(self.data[idx])
        safe_context_str = row['context']
        ...
        context_ids = self.tokenizer.encode(safe_context_str, add_special_tokens=False)
        ...
        content = row['content']
        main_ids = self.tokenizer.encode(content, add_special_tokens=False)
        if len(main_ids) > 32 and random.random() < self.fim_rate:
            content, mid_idx = self._apply_fim_original(content)
            main_ids= self.tokenizer.encode(content, add_special_tokens=False)
            main_labels = main_ids.copy()
            for i in range(0, mid_idx+1):
                main_labels[i] = -100 
        else:
            main_labels = list(main_ids)

        main_ids = main_ids + [self.eos_token_id]
        main_labels = main_labels + [self.eos_token_id]

        input_ids = context_ids + main_ids
